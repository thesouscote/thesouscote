...
(function () {
      ...
      projectsGrid.innerHTML = list.map((p) => {
        ...
        <a href="project-detail.html#${p.id}" class="card project-card reveal" style="text-decoration: none; color: inherit; display: block;">
          <div class="card-media">
            ${badgeHTML}
            ${imgUrl ? `<img src="${imgUrl}" alt="${p.title}" loading="lazy" decoding="async" fetchpriority="low" />` : ''}
          </div>
          <div class="card-body">
      ...
      resourcesGrid.innerHTML = filtered.map((r) => {
        ...
        return `
          <a href="market-detail.html#${r.id}" class="card resource-card reveal is-visible" style="text-decoration: none; color: inherit; display: block;">
            <div class="card-media">
              ${badgeHTML}
              ${r.image ? `<img src="${r.image}" alt="${r.title}" loading="lazy" decoding="async" fetchpriority="low" />` : ''}
            </div>
            <div class="card-body">
    ...
    contactForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (json.success) {
          ...
          status.className = 'contact-status is-success bounce';
          contactForm.reset();
        } else {
        ...
        status.className = 'contact-status is-error shake';
      } finally {
        btn.disabled = false;
  ...
  // ============================================================
  // Gestes Mobile (Swipe Back & Enhanced)
  // ============================================================
  let touchStartX = 0;
  let touchEndX = 0;
  let touchStartY = 0;
  ...
  function handleSwipeGesture() {
    ...
    // Swipe gauche pour ouvrir le menu mobile
    if (deltaX < -100 && Math.abs(deltaY) < 50) {
      const menuToggle = document.querySelector('.menu-toggle');
      const navLinks = document.querySelector('.nav-links');
      if (menuToggle && navLinks && !navLinks.classList.contains('is-open')) {
        menuToggle.click();
      }
    }
  }
  
  // Pull-to-refresh prevention sur mobile
  document.body.addEventListener('touchmove', function(e) {
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    if (scrollTop === 0 && e.touches[0].clientY > 0) {
      // Empêcher le pull-to-refresh uniquement en haut de page
      // e.preventDefault(); // Commenté pour permettre le scroll naturel
    }
  }, { passive: true });
